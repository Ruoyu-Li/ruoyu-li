...
\item A paper about IoT device fingerprinting in high-speed ISP networks is submitted to USENIX Security 2023 (first author).
\item A paper about IoT digital twins and synthetic traffic generation is submitted to JSAC (first author).
\item A paper about real-time malicious traffic detection by programmable switches is submitted to USENIX Security 2023 (co-author).
\item A paper about privacy snooping attacks on smart homes by Wi-Fi traffic is submitted to IMWUT and under revision (co-author).
\item A paper about anomaly detection on smart home devices is submitted to DSN 2023 (co-author).
\item A paper about traffic classification by programmable switches is submitted to TDSC (co-author).
